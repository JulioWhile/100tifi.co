	...
	function (n, e, a) {
		...
		const l = {
			...
			'/:id': async () => {
				...
				return `\n        <div class="characters-inner">\n            <article class="characters-card shadow">\n                <img src="${a}" alt="${t}" />\n                <h2>${t}</h2>\n            </article>\n            <article class="characters-card">\n                <h3>Episodes: <small>${r.length}</small></h3>\n                <h3>Status: <small>${s}</small></h3>\n                <h3>Species: <small>${o}</small></h3>\n                <h3>Gender: <small>${l}</small></h3>\n                <h3>Origin: <small>${d.name}</small></h3>\n                <h3>Last Location: <small>${h.name}</small></h3>\n            </article>\n        </div>\n    `;
			},
			'/contact': 'Contact',
		};
		var d = async () => {
			const n = document.getElementById('header'),
				e = document.getElementById('main__content');
			n.innerHTML = await t();
			let a = c(),
				r = await o(a),
				i = l[r] ? l[r] : s;
